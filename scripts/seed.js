...
function seedWebsite() {
  ...
  const svc = db.prepare(`INSERT INTO web_services (name, trade, summary, body, icon, price_hint, sort)
    VALUES (?,?,?,?,?,?,?)`);
  [
  ...
  const step = db.prepare('INSERT INTO web_steps (step_no, title, body, icon, sort) VALUES (?,?,?,?,?)');
  [
    ...
    [6, '保固售後服務', '施工保固一年，保固期內非人為因素免費回修。', '🛡️', 6]
